#!/usr/bin/env python3
"""
Classify posts in data/test.xlsx using OpenRouter (openai/gpt-5) with the Chinese prompt.
- Per-row error handling and outer try/finally to save partial results.
- Output columns: id, predicted (0=UPWARD,1=NEUTRAL,2=DOWNWARD), gt (blank), raw.
"""
import json
import os
import time
...
TEMPERATURE = 0.1
REQUEST_TIMEOUT = 180  # allow longer timeouts for GPT-5
MAX_TOKENS = 128  # must be >=16 per provider constraints
MAX_ROWS = int(os.getenv("TEST_LIMIT_ROWS", "0"))  # set env TEST_LIMIT_ROWS to limit rows; default full run

EXCEL_PATH = Path(__file__).resolve().parent / "data" / "test.xlsx"
OUTPUT_PATH = Path(__file__).resolve().parent / "outputs" / "test_gpt5_cn.csv"
CONTENT_COLS = ["content", "内容"]  # checked in order
...
def make_client() -> OpenAI:
    ...
    # OpenRouter recommends providing at least a title; referer optional.
    kwargs["default_headers"] = {
        "X-Title": "test_gpt5_cn_run",
    ...
    for attempt in range(3):
        try:
            completion = client.chat.completions.create(
                ...
                # Use minimal reasoning (provider-supported) to avoid long thinking traces.
...
def main() -> None:
    ...
    try:
        df = load_posts()
        client = make_client()
        for idx, row in tqdm(df.iterrows(), total=len(df), desc="Classifying test.xlsx (gpt-5)"):
            # optional pacing if needed: if idx % 100 == 0 and idx > 0: time.sleep(10)
            try:
